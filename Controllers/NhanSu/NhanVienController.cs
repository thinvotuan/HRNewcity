...
namespace BatDongSan.Controllers.NhanSu
{
    public class NhanVienController : ApplicationController
    {
        ...
        public ActionResult MyProfile() {
            ...
            // get hop dong moi nhat
            var hopDongMoiNhat = context.sp_NS_HopDongNhanVien(GetUser().manv, DateTime.Now.Month, DateTime.Now.Year).FirstOrDefault();
            if (hopDongMoiNhat == null) hopDongMoiNhat = new sp_NS_HopDongNhanVienResult();
            model.tongLuong = hopDongMoiNhat.tongLuong;
            return View(model);
        }
        //
        // GET: /NhanVien/Create
        ...
        public ActionResult UpdateNhomUser(int userId, string maNhomUser)
        {
            try
            {
                if (maNhomUser == "Admin")
                {
                    int countNhanSu = contextHTNew.sp_Sys_User_Index("Admin", null, null, null, null, null).Where(d => d.manv == GetUser().manv).Count();
                    if (countNhanSu <= 0)
                    {
